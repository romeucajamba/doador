...
export const StockManagement = () => {
  ...
  if (isLoading) return <div className="p-8 text-center font-bold animate-pulse text-slate-500">A carregar stock global...</div>;
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-neutral-text font-medium text-lg">
          Visão geral de todas as bolsas de sangue disponíveis na rede hospitalar.
        </p>
      </div>
      ...
      <Card className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm border-slate-200/60 dark:border-slate-800">
        <CardHeader>
          <CardTitle className="text-xl font-black text-primary flex items-center gap-2">
            Lotes Registados
            <Badge variant="outline" className="ml-2 font-bold">{stock?.length || 0}</Badge>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="bg-white dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800 overflow-hidden">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {stock?.map((s: any) => (
                  <tr key={s.id_stock} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-colors">
                    <td className="px-4 py-3 font-medium text-slate-600 dark:text-slate-300">#{s.id_stock}</td>
                    ...
                    <td className="px-4 py-3 font-bold text-dark-text dark:text-white">{s.quantidade_bolsas} Bolsas</td>
                    ...
                    <td className="px-4 py-3 text-slate-500">
                      {s.ultima_atualizacao ? new Date(s.ultima_atualizacao).toLocaleString() : 'N/A'}
                    </td>
                  </tr>
                ))}
