...
export const DonorManagement = () => {
  ...
  const handleActivate = async (id: number) => {
    if (
      confirm(
        'Tem a certeza que deseja ativar este doador? Ele poderá aceder ao sistema normalmente.'
      )
    ) {
  ...
  const handleBlock = async (id: number) => {
    if (
      confirm(
        'Tem a certeza que deseja BLOQUEAR este doador? Ele não poderá fazer login nem doar.'
      )
    ) {
  ...
  const renderTable = (
    ...
    title: string,
    emptyMsg: string,
    statusType: 'ativo' | 'inativo' | 'bloqueado'
  ) => (
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-neutral-text font-medium text-lg">
          Gerencie o acesso e o estado de todos os doadores registados no
          sistema.
        </p>
      </div>
