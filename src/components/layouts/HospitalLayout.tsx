    ...
    <div className="flex min-h-screen w-full bg-slate-50 dark:bg-slate-950 font-sans">
      ...
      <aside className="hidden md:flex w-64 flex-col bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 sticky top-0 h-screen">
        <div className="p-6">
          <h1 className="text-xl font-black text-hospital flex items-center gap-2">
            <div className="size-8 bg-hospital rounded-lg flex items-center justify-center text-white">B+</div>
            BLOOD HUB
          </h1>
        </div>

        <nav className="flex-1 px-4 space-y-2">
          <SideNavItem to="/hospital/dashboard" icon={<MdDashboard />} label="Dashboard" />
          <SideNavItem to="/hospital/stock" icon={<MdInventory />} label="Blood Stock" />
          <SideNavItem to="/hospital/donors" icon={<MdGroupAdd />} label="Manage Donors" />
          <SideNavItem to="/hospital/messages" icon={<MdChat />} label="Messages" />
          <SideNavItem to="/hospital/settings" icon={<MdSettings />} label="Settings" />
        </nav>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <header className="sticky top-0 z-10 flex h-16 items-center justify-between bg-white/80 dark:bg-slate-900/80 backdrop-blur-md px-6 border-b border-slate-200 dark:border-slate-800">
          <h2 className="text-lg font-bold text-slate-800 dark:text-white md:hidden">Blood Hub</h2>
          <div className="hidden md:block">
            <h2 className="text-sm font-medium text-slate-500">Welcome back,</h2>
            <p className="text-base font-bold text-slate-900 dark:text-white">{hospital?.name || 'Administrator'}</p>
          </div>
        ...
        <nav className="fixed bottom-0 left-0 right-0 md:hidden bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 p-2 shadow-2xl">
          <div className="grid grid-cols-4 gap-1">
            <MobileNavItem to="/hospital/dashboard" icon={<MdDashboard />} label="Dashboard" />
            <MobileNavItem to="/hospital/stock" icon={<MdInventory />} label="Stock" />
            <MobileNavItem to="/hospital/donors" icon={<MdGroupAdd />} label="Donors" />
            <MobileNavItem to="/hospital/messages" icon={<MdChat />} label="Chat" />
          </div>
        </nav>
      </div>
    </div>
